...
cv::Mat CWarpingModule::FrontalFace() {
	return m_frontalFrame; 
}

PointSetd CWarpingModule::WarpedLandmark() {
	return m_warpLandmarks; 
}

void CWarpingModule::DrawBox( Mat& _img, Point2f* _pvec, Scalar _s, int _thickness) {
	//_tl, _bl, _tr, _br,
	Point2i tl = _pvec[0]; 
	Point2i bl = _pvec[1]; 
	Point2i tr = _pvec[2]; 
	Point2i br = _pvec[3]; 
	line(_img, tl, bl, _s, _thickness, CV_AA ); 
	line(_img, bl, br, _s, _thickness, CV_AA ); 
	line(_img, br, tr, _s, _thickness, CV_AA ); 
	line(_img, tr, tl, _s, _thickness, CV_AA ); 
}
...
//    SHOW_IMG(_img);
//    int idx[9] = {20, 23, 26, 29, 15, 17, 19, 32, 38}; // Matlab idx
//	PointSetd pntsall = CWarpUtils::Mat2Pnts2(_pnts);
//    int nPnts =
    PointSetd srcPnts = _pnts;
    PointSetd tgtPnts(NUM_POINTS);
    vector<Point3d> tgtPnts3d(NUM_POINTS);
    FOR (i, NUM_POINTS) {
//        srcPnts[i] = _pnts[i];
	...
	Mat warp2d = Mat::zeros(_img.size(), _img.type()); 
	warpAffine(_img, warp2d, m_H, _img.size()); 
...
//    SHOW_IMG(warp2d);
	if (_isWarp)
		m_frontalFrame = WarpFace(warp2d);
	else
		m_frontalFrame = Mat();
}
			...
			if (m_Z.at<double>(Point(w, h)) > 0) {
				...
				if (nx > 0 && ny > 0 && nx < warpWidth-1 && ny < warpHeight-1) {
					...
					/*for (int ll = 0; ll < 3; ll++) {
						warp3d.at<Vec3b>(Point(w,h))[ll] = warp2d.at<Vec3b>(nyi, nxi)[ll];
					}*/
					int nxi1 = nxi+1;; 
					int nyi1 = nyi+1; 
