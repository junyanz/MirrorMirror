...
CCLMTrackingModule::CCLMTrackingModule() {
    ...
    m_failed = false;
    m_fpd = -1;
    m_model = new FACETRACKER::Tracker ("./models/face2.tracker");
    m_tri=FACETRACKER::IO::LoadTri("./models/face.tri");
    m_con=FACETRACKER::IO::LoadCon("./models/face.con");
}
...
bool CCLMTrackingModule::TrackFrame( const Mat& _frame, bool _isVis /*= false*/ ) {
	...
	//track this image
    std::vector<int> wSize; if(m_failed)wSize = wSize2; else wSize = wSize1;
    if(m_model->Track(gray,wSize,m_fpd,nIter,clamp,fTol,fcheck) == 0){
        int idx = m_model->_clm.GetViewIdx(); m_failed = false;
		Draw(im,m_model->_shape,m_con,m_tri,m_model->_clm._visi[idx]); 
	} else{
		if(_isVis){cv::Mat R(im,cvRect(0,0,150,50)); R = cv::Scalar(0,0,255);}
        m_model->FrameReset(); m_failed = true;
	}     
